...
def main(img):
    ...
    (labels, output_img,l,mat_array) = run(rotated)
   ...
   # print X,Y
    ...
    for (x,y) in labels:
        if find(lab,labels[x,y]):
            cv2.putText(img,str(mat_array.index(labels[x,y])),(x-15,y+10),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),2)
            lab.append(labels[x,y])
    ...
    rot = output_img.rotate( 270, expand=1 )
    #img = np.array(rot)
